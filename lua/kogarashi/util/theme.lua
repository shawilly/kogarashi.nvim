...
---@class kogarashi.util.theme
local M = {}

local function load_autocmds()
  local ok, _ = pcall(require, "kogarashi.autocmds")
  if not ok then
    Util.log("Failed to load kogarashi.autocmds", "error")
  end
end
...
local function get_real_color(hex_color, base)
  ...
  local c = require("kogarashi.colorscheme.palette.kogarashi")
  if base == nil then
    base = c.background
  end
...
function M.load(hl_group_tbl)
  ...
  vim.g.colors_name = "kogarashi"

  M.draw(hl_group_tbl)

  local bufferline_icon_hl_group_tbl = require("kogarashi.theme.plugins.bufferline").setup_bufferline_icon()
